...
* rotate keys
#+begin_verse
if the user is admin (detect using by checking if their id is '195391705' in  function `user_gemini_rotate_keys_p` fn), and GEMINI_ROTATE_KEYS_P == True (add to  constants), instead of using the configured api key, rotate keys from  GEMINI_API_KEYS=~/.gemini_api_keys (add to constants). 
#+end_verse
