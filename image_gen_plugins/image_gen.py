...
LOG_DIR = Path(os.path.expanduser("~/.borg/image_gen/log/"))
...
BOT_COMMANDS = [
    ...
    {
        "command": "setgeminikey",
        "description": "Set Gemini API key for image generation",
    },
...
async def _log_image_generation(
    event, prefs: ImageGenPrefs, prompt: str, images_generated: int
):
    """Formats and writes the image generation log to a user-specific file."""
    try:
        ...
        print(f"Failed to write image generation log for user {event.sender_id}: {e}")
        traceback.print_exc()
    ...
    try:
        ...
        if images:
            await status_msg.edit(
                f"{BOT_META_INFO_PREFIX}✅ Generated {len(images)} image(s):"
            )
            ...
            if log_p:
                prefs = user_manager.get_prefs(user_id)
                await _log_image_generation(event, prefs, prompt, len(images))
        else:
...
def register_handlers():
    ...
    borg.on(events.NewMessage(func=lambda e: e.is_private and not e.forward))(
        message_handler
    )
