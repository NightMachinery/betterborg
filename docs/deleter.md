...
- `.delalltext N` scans up to the last `N` messages in the current chat and
  deletes text-only messages regardless of sender, except messages whose text is
  only whitespace-separated hashtags.
...
non-empty text and no Telethon media object. Messages such as
