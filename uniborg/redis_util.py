...
REDIS_LONG_EXPIRE_DURATION = int(
    os.environ.get("BORG_REDIS_LONG_EXPIRE_DURATION", "2592000")
)  # 1 month default
REDIS_VERY_LONG_EXPIRE_DURATION = int(
...
async def hset_with_expiry(
    key: str, field_values: dict, *, expire_seconds: int = None
) -> bool:
...
async def zadd_with_limit_and_expiry(
    key: str, score_member_pairs: dict, *, limit: int = None, expire_seconds: int = None
) -> bool:
...
async def zrange_and_renew(
    key: str, start: int = 0, end: int = -1, *, expire_seconds: int = None
) -> list:
