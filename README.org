...
** deleter
=stdplugins/deleter.py= adds the admin-only =.del N= and =.del s N=
commands. The self-only form deletes the userbot account's own messages and
selectively clears that account's reactions from scanned messages. See
