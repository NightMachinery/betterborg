...
=.delalltext=, =.delallself=, and =.delallselfreactions= commands. The self-only
...
text-only posts except hashtag-only marker posts. The bulk self-delete command
clears all of the account's reactions before deleting all of its messages; the
reaction-only bulk command is available separately. See
