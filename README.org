...
The current adapter accepts image inputs but returns text only. Codex's built-in
subscription image generation is not yet integrated; see the image-generation
section in [[file:docs/codex_models.md][docs/codex_models.md]] for the distinction.
