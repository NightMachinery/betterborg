...
** deleter
=stdplugins/deleter.py= adds the admin-only =.del N=, =.del s N=, and
=.delallself= commands. The self-only form deletes the userbot account's own
messages and selectively clears that account's reactions from scanned messages.
The bulk self-delete command uses Telegram's participant-history deletion. See
