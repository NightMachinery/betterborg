...
=.delallself=, and =.delallselfreactions= commands. The self-only form deletes
the userbot account's own messages and selectively clears that account's
reactions from scanned messages. The bulk self-delete command clears all of the
account's reactions before deleting all of its messages; the reaction-only bulk
command is available separately. See
