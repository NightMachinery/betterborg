...
The current adapter accepts image inputs but returns text only. Image generation
through the same OAuth endpoint passed a live test, but Telegram delivery is not
yet integrated; see [[file:docs/codex_models.md][docs/codex_models.md]] for the results.
